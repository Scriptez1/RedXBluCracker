...
try:
  import requests
    ...
    print(f"{Fore.RED}Modul Eksik - no such module")
    print("Yukleniyor... - Loading...")
    os.system("pip install colorama")
    os.system("pip install requests")
    print(f"{Fore.GREEN}Yükleme Tamamlandı! Yeniden Açın. - Complated! Restart Code.")
    time.sleep(1.5)
    exit(0)
...
BluTv Pin Kırıcı {Fore.RESET}- {Fore.RED}FuryLemons 

""")

mail = input(Fore.RED + "Mail-> " + Fore.RESET)
passw = input(Fore.RED + "Şifre - Password-> " + Fore.RESET)
...
def get_profiles(token):
    ...
    for i,profile in enumerate(responseAry):
        if profile["has_pin"]:
            ...
            name = profile["name"]
            ids.append(_id)
            print(f"{Fore.RED}{i+1}> {Fore.RESET}{name}")
    selected = int(input(Fore.RED + "Select Profile-> " + Fore.RESET)) - 1
    global selected_id
...
def main():
    ...
    thread_bots = int(input(Fore.RED + "Bots-> " + Fore.RESET))
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=thread_bots) as executor:
        ...
        for future in concurrent.futures.as_completed(response):
            ...
            if result:
                ...
                attempt_counter += 1
                print(f"\r{Fore.RED}Attempts: {Fore.BLUE}{attempt_counter} {Fore.RESET}/ {Fore.GREEN}9999", end='', flush=True)
                print(Fore.RESET)
